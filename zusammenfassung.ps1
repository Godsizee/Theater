...
# Schließt jetzt sowohl "lib" als auch "node_modules" aus.
# Das Muster verwendet `|` als "ODER"-Operator in der Regex.
$ExcludeDirectoryPattern = "lib|node_modules"

# Dateitypen, die verarbeitet werden sollen
$FileTypesToProcess = "*.php", "*.html", "*.css", "*.js", "*.json", ".htaccess", "*.sql"
...
Write-Host "Erstelle Datei mit der kompletten Ordnerstruktur: '$structureFilePath'"

# Alle Dateipfade sammeln
$allFilePaths = Get-ChildItem -Path $SourceDirectory -Recurse -File | Select-Object -ExpandProperty FullName
